...
def test_exact_tcp_and_udp_lookup(tmp_path):
    events = [
        ...
        {"type": "tcp_proxy_dial", "conn_id": "t1", "outer_conn_id": "o1", "leaf_proxy_type": "VLESS", "post_flow": _flow("tcp", "192.0.2.1", 50000, "203.0.113.1", 443, "physical")},
        {"type": "udp_connect", "conn_key": "u1", "pre_flow": _flow("udp", "2001:db8::1", 53000, "2001:db8::2", 443, "logical")},
        {"type": "udp_proxy_dial", "conn_key": "u1", "outer_conn_id": "o2", "leaf_proxy_type": "AnyTLS", "post_flow": _flow("udp", "192.0.2.1", 51000, "203.0.113.2", 443, "physical")},
    ]
    ...
    assert tcp[0].leaf_proxy_type == "VLESS"
    assert udp[0].post_flow.src_port == 51000
    assert udp[0].leaf_proxy_type == "AnyTLS"
