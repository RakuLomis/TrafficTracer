...
TrafficTracer Complete is developed from the `Complete` branch with pinned recursive submodules. The supported 1.0.0 release target is `x86_64-unknown-linux-gnu`.
...
The toolchain check covers Python 3.12 or newer, Go, Rust/Cargo, pnpm, PyInstaller, Chrome/Chromium, tshark, and dumpcap. Release 1.0.0 is built and validated in the current Ubuntu 24.04 LTS environment.
...
The 1.0.0 Linux Deb retains the upstream package identity for upgrade compatibility while using bundle version:

```text
2.5.2+traffictracer.1.0.0
```
