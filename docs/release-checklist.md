...
- [ ] Interrupt one batch; confirm cleanup, no later target starts, and Resume retries the exact interrupted target.
- [ ] Inject a child failure, restart the Worker, and resume from the exact failed target.
