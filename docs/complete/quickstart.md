...
TrafficTracer Complete 1.0.7 packages the pinned UI, Worker, Mihomo core, and privileged service integration as one Linux x86-64 application. This guide covers the normal desktop workflow. It does not require sibling repositories or standalone capture commands.
...
```text
traffictracer-complete-v1.0.7-linux-x86_64/
...
```bash
cd /path/to/traffictracer-complete-v1.0.7-linux-x86_64
sha256sum -c SHA256SUMS
```

`VERSION` identifies TrafficTracer Complete 1.0.7. `COMPONENTS` records the exact TrafficTracer, Mihomo, UI, and service revisions used by the package.
...
`complete/components.lock.yaml` is authoritative. TrafficTracer Complete 1.0.7 uses:
