...
TrafficTracer Complete 1.0.27 packages the pinned UI, Worker, Mihomo core, and privileged service integration as one Linux x86-64 application. This guide covers the normal desktop workflow. It does not require sibling repositories or standalone capture commands.
...
```text
traffictracer-complete-v1.0.27-linux-x86_64/
├── TrafficTracer-Complete_1.0.27_linux_x86_64.deb
├── TrafficTracer-Complete_1.0.27_linux_x86_64.AppImage
├── SHA256SUMS
├── VERSION
├── COMPONENTS
├── SBOM.cdx.json
...
```bash
cd /path/to/traffictracer-complete-v1.0.27-linux-x86_64
sha256sum -c SHA256SUMS
```

`VERSION` identifies TrafficTracer Complete 1.0.27. `COMPONENTS` records the exact TrafficTracer, Mihomo, UI, and service revisions used by the package.
...
```bash
sudo apt install ./TrafficTracer-Complete_1.0.27_linux_x86_64.deb
```

The package name remains `clash-verge` and its bundle version is `2.5.2+traffictracer.1.0.27`. This allows an in-place upgrade of an existing Clash Verge installation and preserves user configuration. The already-running process does not change until it exits. Use a maintenance window, exit it normally, install the package, and start the new version.

To avoid installing system files, use the AppImage:

```bash
chmod +x TrafficTracer-Complete_1.0.27_linux_x86_64.AppImage
./TrafficTracer-Complete_1.0.27_linux_x86_64.AppImage
```
...
Current packages compress new analysis snapshots using lossless gzip, without
...
`complete/components.lock.yaml` is authoritative. TrafficTracer Complete 1.0.27 uses:
