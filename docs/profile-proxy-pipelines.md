...
```text
pipeline -> repetition -> target -> profile/selector/node candidate -> capture-only Session
         -> repetition capture barrier -> serial analysis wave
```

The stable candidate identity is `(profile_uid, selection_group, requested_node)`.
Each matrix cell records a one-based `repetition_index`, the frozen target
index, and `candidate_ordinal`; its durable logical identity is
`(repetition_index, target_index, candidate_ordinal)`. Node names alone are
not unique and do not identify the selector whose state must be changed. Every
...
## Capture and analysis barriers

Before any Profile or node is changed, every distinct target's capture-only Job
is validated by the Worker without creating a Batch, Session, directory or
browser process. Shared contract/API failures stop the entire Pipeline
immediately even when continue-on-run-failure is enabled.

For each repetition the supervisor walks targets in their frozen YAML order.
For one target it visits every candidate in the repetition's frozen candidate
order. Before each cell it activates the Profile, waits for the real Controller
API, selects and reads back the requested node, resolves the leaf chain, records
the current connection count without closing any global connections, and
performs environment checks. It then
creates exactly one capture-only Batch and one raw Session. A checkpoint must
be durable before the next candidate is selected.

No analysis Job starts while any initial cell in that repetition remains
pending, running, or interrupted. After the complete capture wave is durable,
the supervisor analyzes the captured Sessions serially. Only after the analysis
wave (and any bounded deferred retry wave) is terminal may the next repetition
begin.
...
records the Controller connection inventory as `preserved`; it does not delete
connections belonging to other applications. Per-cell browser process cleanup
provides the capture boundary. A second chain snapshot immediately before Batch
start must match the first snapshot exactly.
...
New pipelines use repetition-target-candidate order. One seeded, balanced
candidate permutation is frozen per repetition and reused for every target in
that repetition. Candidate order may rotate between repetitions. Existing
schema-v6 and older manifests retain candidate-major execution and remain
resumable. An interrupted capture resumes its existing Batch checkpoint. An
interrupted analysis returns to `captured` and reanalyzes the existing raw
Session without recapturing it.

A semantic configuration change after materialization raises
`CANDIDATE_CONFIG_DRIFT`; affected cells are failed or skipped explicitly.
Deleted Profiles and missing nodes use the same candidate circuit breaker.
Original Profile and selector state is restored on every terminal path.
...
Every matrix cell owns a separate run directory and capture-only Batch checkpoint; the same URL therefore remains distinguishable across repetitions and candidates. `pipeline-aggregate.json` schema v2 separately reports repetition completion and cell completion, plus per-candidate Session totals and Capture/Correlation/Application counters.
...
`pipeline-manifest.json` schema v7 records the frozen scheduling seed and per-repetition candidate orders, cell capture/analysis states, Session history, durable Profile activation and configuration-binding evidence, plus three independent quality planes for
each terminal run:
